...
*
* Overview:
*   This file is an implementation of a heap-based priority queue and the heapsort algorithm. 
*
* Input:
...
// prototype to indexSmallest function
...
void adjustHeap(DynamicArray* heap, int last, int position, compareFunction compare)
{
    ...
    if(rightChild < last){
        ...
        small = indexSmallest(heap, leftChild, rightChild, compare);
        // if the value of child is smaller than value of parent
...
void dyHeapRemoveMin(DynamicArray* heap, compareFunction compare)
{
    ...
    // percolate down (recursive function)
